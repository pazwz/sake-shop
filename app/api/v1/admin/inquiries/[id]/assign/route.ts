...
import { inquiryAssignmentValidator } from '@/validators/contact-inquiry.validator';
...
export const PATCH = async (
  ...
  try {
    ...
    const input = inquiryAssignmentValidator.parse(await request.json());
    return createSuccessResponse(
      await service.assign((await params).id, input.assignedAdminId, admin),
    );
  } catch (error) {
