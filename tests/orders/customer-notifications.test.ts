...
import { getCustomerMessageSenderName } from '@/components/customer-order-messages';
...
test('customer message sender uses the customer profile name with a safe fallback', () => {
  assert.equal(getCustomerMessageSenderName('山田 太郎'), '山田 太郎様');
  assert.equal(getCustomerMessageSenderName('  '), 'お客様');
});
...
test('customer notification and order-message pages retain distinct notification groups and conversation semantics', async () => {
  ...
  assert.match(notificationsPage, /tab=personal/);
  assert.match(notificationsPage, /tab=site/);
  assert.match(notificationsPage, /aria-current/);
